...
import AnimatedYellowBird from '../../Components/AnimatedYellowBird';
...
// Hook for screen bouncing logic
...
// NEW Hook for smooth wing flapping animation
const useFlapAnimation = (speed = 0.1) => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            frame.current += 1;
            // Use Math.sin for a smooth up-and-down cycle
            setFlap(Math.sin(frame.current * speed));
...
// --- The Flapping Paper Bird Component ---
const AnimatedPaperBird = memo(({ size, startX, startY, others = [] }) => {
    const pos = useBounce(startX, startY, size, 0.5, others);
    const flap = useFlapAnimation(0.15); // Controls flap speed

    const color1 = "#FFDD00"; // Main top-left wing
    const color2 = "#FFC700"; // Thin middle fold/shadow
    const color3 = "#B39B00"; // Darker bottom wing

    // Define the range of motion for the wings
    const flapRange = 10;
    
    // Calculate the new Y positions for the wing tips based on the flap state
    const topWingY = 0 - (flap * flapRange);
    const bottomWingY = 94.1 + (flap * flapRange);
    
    // Create the dynamic points strings for the SVG polygons
    const topWingPoints = `23,${topWingY} 84.7,46.1 42.3,53.2`;
    const bottomWingPoints = `0,${bottomWingY} 84.7,53.2 42.3,53.2`;
    const middleShardPoints = "84.7,46.1 84.7,53.2 42.3,53.2";
    ...
    return (
        <div className="fixed pointer-events-none z-0" style={{
            ...
            <div style={{ width: '100%', height: '100%' }}>
                <svg
                    viewBox="0 0 84.7 94.1"
                    width="100%"
                    height="100%"
                    preserveAspectRatio="xMidYMid meet"
                    style={{ filter: `drop-shadow(0 0 8px rgba(0,0,0,0.3))` }}
                >
                    <polygon fill={color1} points={topWingPoints} />
                    <polygon fill={color2} points={middleShardPoints} />
                    <polygon fill={color3} points={bottomWingPoints} />
                </svg>
            </div>
        </div>
    );
...
const FlyingBird = memo(() => {
    ...
    const birdConfigs = [
        { size: 288, startX: window.innerWidth * 0.8, startY: window.innerHeight * 0.8 },
        { size: 192, startX: window.innerWidth * 0.2, startY: window.innerHeight * 0.2 },
        { size: 144, startX: window.innerWidth * 0.6, startY: window.innerHeight * 0.4 },
        { size: 120, startX: window.innerWidth * 0.4, startY: window.innerHeight * 0.6 },
        { size: 96,  startX: window.innerWidth * 0.2, startY: window.innerHeight * 0.8 },
    ];

    const birds = birdConfigs.map((cfg, idx) => (
        <AnimatedYellowBird key={idx} {...cfg} />
    ));

    return (
        <>
            {/* The old 'spin' animation is no longer needed */}
            <div
...
FlyingBird.displayName = 'BuilderIoPage';
